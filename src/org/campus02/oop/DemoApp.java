...
public class DemoApp {
	...
	public static void main(String[] args) {
		...
		pm.doAnalysis(ca);
		HashMap<String ,Integer> caResult = ca.getResult();
		for (String country : caResult.keySet()) {
			System.out.println(country + " : " + caResult.get(country));
		}
		MaxSalaryAnalyzer msa = new MaxSalaryAnalyzer();
		pm.doAnalysis(msa);
		BMIAnalyzer bmi = new BMIAnalyzer();
		pm.doAnalysis(bmi);
		ArrayList<Pair<Person, Double>> bmiResult = bmi.getResult();
		for (Pair<Person, Double> pp:bmiResult) {
